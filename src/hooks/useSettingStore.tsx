...
export const useSettingStore = create<ISettingStoreState>()((set) => ({
    ...
    city: defaultSettings.city ?? "Quy Nhon",
    setCity: (newCity) => {
        set({city: newCity})
    },
    aiEnabled: defaultSettings.aiEnabled ?? true,
    setAiEnabled: (newBoolean) => {
        set({aiEnabled: newBoolean})
    },
    aiFrequencyMinutes: defaultSettings.aiFrequencyMinutes ?? 3,
    setAiFrequencyMinutes: (frequency) => {
        set({aiFrequencyMinutes: frequency})
    },
